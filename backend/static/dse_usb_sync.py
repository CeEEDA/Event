...
def sync_to_portal(db_conn, api_url, device_id, device_key):
    ...
    for row_id, data_json in rows:
        ...
        # Alarms aus dem Record extrahieren - aber NICHT direkt anhaengen,
        # sondern pro Record getrennt sammeln. Aktive Alarme bestimmen wir am
        # Ende nur aus dem NEUSTEN Record - aelterer Buffer-Inhalt darf einen
        # bereits geloeschten Alarm nicht wieder "aktiv" halten.
        rec_alarms = []
        ...
        all_alarms.append((raw.get("timestamp", ""), rec_alarms))

    # Aktuell-aktive Alarme = Alarme des NEUSTEN Records. Wenn der neuste
    # Record keine Alarme hat -> leere Liste -> Backend resolved alle offenen.
    if all_alarms:
        all_alarms.sort(key=lambda x: x[0])
        latest_alarms = all_alarms[-1][1]
    else:
        latest_alarms = []
    # Per-pos dedup (sollte sowieso unique sein, aber zur Sicherheit)
    _seen = {}
    for a in latest_alarms:
        _seen[a.get("alarm_code")] = a
