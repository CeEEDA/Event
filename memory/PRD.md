...
- 2026-05-22: **UX Energy Monitoring Datenpunkte (P2):** Endpoint `/api/energy-monitoring/devices/{id}/telemetry` lieferte vorher nur die juengsten 5000 Records via `sort desc + limit` → bei mehr als 5000 Records wurde der Anfang des Tages abgeschnitten. Backend macht jetzt gleichmaessig verteiltes Downsampling (jedes N-te Record + letzten Punkt mitnehmen) und gibt `{items, total, downsampled}` zurueck. Frontend zeigt `"X von Y Datenpunkten (gleichmaessig verteilt)"` damit der User sieht wie viele Records wirklich existieren. CSV-Export weiterhin ungekuerzt.
