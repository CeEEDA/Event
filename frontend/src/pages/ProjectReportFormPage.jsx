    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <fieldset disabled={locked} className="max-w-4xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4" data-testid="section-material">
          <div className="flex items-center justify-between mb-3 sticky top-0 z-20 bg-white py-2 -mx-4 px-4 -mt-4 pt-4 border-b border-gray-100 rounded-t-lg">
            <h2 className="text-sm font-semibold text-gray-700 flex items-center gap-2">
              <Wrench className="w-4 h-4 text-amber-500" /> Material / Artikel
              {material.length > 0 && (
                <span className="text-[10px] font-normal text-gray-400 ml-1">({material.length})</span>
              )}
            </h2>
          ...
          {/* Bequemer Add-Button unter dem letzten Row - kein Zurueckscrollen noetig */}
          <button
            type="button"
            onClick={() => setMaterial(m => [...m, emptyMaterial()])}
            className="mt-2 w-full flex items-center justify-center gap-1 py-2 text-xs text-gray-500 hover:text-amber-600 border border-dashed border-gray-300 hover:border-amber-400 rounded-md transition-colors"
            data-testid="add-material-bottom"
          >
            <Plus className="w-3.5 h-3.5" /> Weitere Zeile hinzufügen
          </button>
        </div>
