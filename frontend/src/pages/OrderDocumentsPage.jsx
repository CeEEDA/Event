...
import {
  ArrowLeft, Upload, FileText, Image, Trash2, Download, Eye,
          ...
          <>
            ...
            {folderDocs.length === 0 ? (
              ...
              <div className="space-y-2">
                {folderDocs.map(doc => (
                  <DocRow key={doc.id} doc={doc} pk={pk} isAdmin={isAdmin}
                    onDelete={handleDelete} onPreview={setPreviewDoc} onChangeKat={handleChangeKategorie}
                    getFileUrl={getFileUrl} isImage={isImage} formatSize={formatSize} />
                ))}
              </div>
            )}
          </>
                ...
                <div key={idx} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      updated[idx] = { ...pf, kategorie: e.target.value };
                      setPendingFiles(updated);
                    }}
                    className="text-sm border border-gray-300 rounded-lg px-2 py-1.5 bg-white focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500"
                    data-testid={`pending-kat-${idx}`}
                  >
                    {KATEGORIEN.map(k => (
                      <option key={k.key} value={k.key}>{k.label}{k.key === pf.detected ? " (erkannt)" : ""}</option>
                    ))}
                  </select>
                </div>
              ))}
...
function DocRow({ doc, pk, isAdmin, onDelete, onPreview, onChangeKat, getFileUrl, isImage: checkImage, formatSize: fmtSize }) {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors" data-testid={`doc-${doc.id}`}>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        <select value={doc.kategorie || "sonstiges"}
          onChange={e => onChangeKat(doc.id, e.target.value)}
          className="text-xs border border-gray-200 rounded px-1.5 py-1 bg-white text-gray-600 mr-1"
          data-testid={`kat-select-${doc.id}`}>
